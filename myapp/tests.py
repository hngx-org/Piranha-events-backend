...
from .models import Event, Comment, Image, User, Group, User_group, InterestedEvent
import datetime
...
class UserManagementAPITest(unittest.TestCase):
    ...
    def test_update_user_profile(self):
        ...
        response = requests.put(
            f"{self.base_url}/profile", json=data, headers=headers
            )
        self.assertEqual(response.status_code, 200)
        response_data = response.json()
...
class EventAPITestCase(TestCase):
    ...
    def test_update_event_details(self):
        ...
        url = reverse("event-detail", args=[event.id])
        data = {
            "title": "Updated Event Name", "description": "Updated description"
            }
    ...
    def test_get_images_for_comment(self):
        ...
        image = Image.objects.create(
            comment=comment, image_url="image_url.jpg"
            )
        url = reverse("image-list", args=[comment.id])
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class CommentViewTestCases(TestCase):
    def setUp(self) -> None:
        ...
        Comment.objects.create(
            body="Comment 1", user=self.user, event=self.event
            )
        Comment.objects.create(
            body="Comment 2", user=self.user, event=self.event
            )
